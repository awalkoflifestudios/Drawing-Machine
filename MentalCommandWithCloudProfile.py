...
while 1:
    ...
    if state == 0: #EDK_OK
        ...
        if EngineEventType == 64: #EmoState Updated
            ...
            if running == 1:
               mental_state_name = IEE_MentalCommandAction_enum[mental_state],
               print "Mental State: ", mental_state, " ", mental_state_name,"\t Power:",power
               current_mental_state[mental_state_name] = power
 
        if EngineEventType == 256: #MentalCommandEvent
            handleMentalCommandEvent(eEvent)

        # move toward each target[x or y axistime.sleep(.015)
        for state_name in IEE_MentalCommandAction_enum:
            if current_pos[ state_name ] != current_mental_state[state_name]:
                # increment the pos & build new gcode
                # if enought time has passed
                # send out gcode if necessary
